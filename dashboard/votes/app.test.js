...
const { parseVotes, buildMessage, extractFirstRoster } = require("./app.js");
...
test("extractFirstRoster: uses the maintained top-of-thread alive list and the latest day as the current night", () => {
  const log =
    // Opening post's alive list, edited down to the current survivors (2 left)
    // - this is the maintained list the tracker should use, even though later
    // vote-count posts repost bigger day-specific lists.
    forumPost("Mod", "May 1, 2026, 12:00:00 AM", "Rules etc.\nAlive Player List\n\nAlice\nBob\n\n") +
    forumPost("Mod", "May 1, 2026, 1:00:00 PM", "Day 1 Start\n\nAlive Player List\n\n1. Alice\n2. Bob\n3. Carol\n4. Dave\n\nWith 4 players alive it will take 3 to achieve majority.") +
    forumPost("Mod", "May 4, 2026, 1:00:00 PM", "Day 4 Start\n\nAlive Player List\n\n1. Alice\n2. Bob\n\nWith 2 players alive it will take 2 to achieve majority.");

  const { roster, night } = extractFirstRoster(log);
  assert.deepEqual(roster, ["Alice", "Bob"]); // the FIRST (maintained) list, not a later day's
  assert.equal(night, 4); // latest "Day N Start" seen
});
