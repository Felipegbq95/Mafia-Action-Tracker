...
// --- Mafia Action Tracker integration ---------------------------------------
// Non-invasive addition: when this app is embedded in an iframe inside the
// Action Tracker dashboard, it also posts the day-boundary timestamps it
// already detects up to the parent window, so the host can optionally
// auto-fill night windows instead of typing start/end times by hand. This
// reuses splitMarkedText/splitForumPosts/SYSTEM_SIGNATURE_RE/DAY_START_RE
// exactly as parseForumThread does, but never touches vote tallying itself -
// standalone use of this app (not embedded) is completely unaffected.
function extractDayStarts(rawText) {
  const { clean: cleanText } = splitMarkedText(rawText);
  const posts = splitForumPosts(cleanText);
  const dayStarts = [];
  for (const post of posts) {
    if (!SYSTEM_SIGNATURE_RE.test(post.content)) continue;
